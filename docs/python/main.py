...
def main() -> None:
    ...
    with request.urlopen("https://dicom.nema.org/medical/dicom/current/output/chtml/part15/chapter_e.html") as response: # noqa: S310
       html = response.read()
    ...
    retain_description_option = ["K" if i == "C" else i for i in df_table["Clean Desc. Opt."].to_list()]
    ...
    file_content = f"const dicomTagToNemaActionObject = {json.dumps(dicom_tag_to_nema_action)};"
    with Path("dist/dicom-tag-to-nema-action.js").open("w", encoding="utf-8") as file:
        file.write(file_content)
    assert sha256(file_content.encode("utf-8")).hexdigest() == "54dfa7ecb18b80983e18f2b3a629b588eb0c9e6f26e6c4b56026321ac4d5baaf"
