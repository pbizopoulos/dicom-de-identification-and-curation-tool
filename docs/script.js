(() => {
	...
	const dicomTagSavePathInputText = document.getElementById(
		"dicom-tag-save-path-input-text",
	);
	const dicomTagValuesRemovedNumSpan = document.getElementById(
		"dicom-tag-values-removed-num-span",
	);
	const dicomTagValuesReplacedNumSpan = document.getElementById(
		"dicom-tag-values-replaced-num-span",
	);
	const filesProcessedNumSpan = document.getElementById(
		"files-processed-num-span",
	);
	const filesSizeTotalSpan = document.getElementById("files-size-total-span");
	const loadDirectoryInputFile = document.getElementById(
		"load-directory-input-file",
	);
	const loadSessionInputFile = document.getElementById(
		"load-session-input-file",
	);
	const patientPseudoIdPrefixInputText = document.getElementById(
		"patient-pseudo-id-prefix-input-text",
	);
	const retainDescriptorsInputCheckbox = document.getElementById(
		"retain-descriptors-input-checkbox",
	);
	const retainDeviceIdentityInputCheckbox = document.getElementById(
		"retain-device-identity-input-checkbox",
	);
	const retainPatientCharacteristicsInputCheckbox = document.getElementById(
		"retain-patient-characteristics-input-checkbox",
	);
	const retainSafePrivateInputCheckbox = document.getElementById(
		"retain-safe-private-input-checkbox",
	);
	const retainUidsInputCheckbox = document.getElementById(
		"retain-uids-input-checkbox",
	);
	const saveProcessedFilesAsZipButton = document.getElementById(
		"save-processed-files-as-zip-button",
	);
	...
	function loadDirectoryInputFileOnInput() {
		...
		if (filesSizeTotal > 1000000000) {
			filesSizeTotalSpan.textContent +=
				": must be less than 1GB, cannot proceed.";
			...
			for (let i = 0; i < filesNum; i++) {
				...
				if (patientIdDicomTag in dicomDictArray[i].dict) {
					...
					if (!(patientId in sessionObject)) {
						const patientPseudoIdBase = Object.keys(
							sessionObject,
						...
						let daysOffset =
							Math.floor(Math.random() * 365 * 10 * 2) - 365 * 10;
					...
					dicomDictArray[i].dict[patientIdDicomTag].Value[0] =
						sessionObject[patientId].patientPseudoId;
					dicomTagValuesReplacedNum++;
					dicomDictArray[i].dict[patientNameDicomTag].Value[0] =
						dicomDictArray[i].dict[patientIdDicomTag].Value[0];
					dicomTagValuesReplacedNum++;
				}
				for (const dicomTag in dicomTagToNemaActionObject) {
					if (dicomTag === patientIdDicomTag) {
						...
						dicomTagToNemaActionObject[dicomTag][1] === "K" &&
						retainSafePrivateInputCheckbox.checked
					) {
					} else if (
						dicomTagToNemaActionObject[dicomTag][2] === "K" &&
						retainUidsInputCheckbox.checked
					) {
					} else if (
						dicomTagToNemaActionObject[dicomTag][3] === "K" &&
						retainDeviceIdentityInputCheckbox.checked
					) {
					} else if (
						dicomTagToNemaActionObject[dicomTag][4] === "K" &&
						retainPatientCharacteristicsInputCheckbox.checked
					) {
						...
						if (dateProcessingSelect.value === "keep") {
							...
							if (dicomTag in dicomDictArray[i].dict) {
								if (dicomDictArray[i].dict[dicomTag].vr === "DA") {
									const year = parseInt(
										dicomDictArray[i].dict[dicomTag].Value[0].slice(0, 4),
									);
									const month =
										parseInt(
											dicomDictArray[i].dict[dicomTag].Value[0].slice(4, 6),
										) - 1;
									const day = parseInt(
										dicomDictArray[i].dict[dicomTag].Value[0].slice(6, 8),
									);
									...
									const monthWithOffset = (
										dateWithOffset.getMonth() - 1
									).toLocaleString("en-US", {
										minimumIntegerDigits: 2,
										useGrouping: false,
									});
									const dayWithOffset = dateWithOffset
										.getDay()
										.toLocaleString("en-US", {
											minimumIntegerDigits: 2,
											useGrouping: false,
										});
									dicomDictArray[i].dict[dicomTag].Value[0] =
										`${yearWithOffset}${monthWithOffset}${dayWithOffset}`;
								} else if (dicomDictArray[i].dict[dicomTag].vr === "TM") {
									const hours = dicomDictArray[i].dict[dicomTag].Value[0].slice(
										0,
										2,
									);
									const minutes = dicomDictArray[i].dict[
										dicomTag
									].Value[0].slice(2, 4);
									const seconds = dicomDictArray[i].dict[
										dicomTag
									].Value[0].slice(4, 6);
									const date = new Date();
									date.setHours(hours);
									...
									date.setSeconds(
										seconds + sessionObject[patientId].secondsOffset,
									);
									const hoursWithOffset = date
										.getHours()
										.toLocaleString("en-US", {
											minimumIntegerDigits: 2,
											useGrouping: false,
										});
									const minutesWithOffset = date
										.getMinutes()
										.toLocaleString("en-US", {
											minimumIntegerDigits: 2,
											useGrouping: false,
										});
									const secondsWithOffset = date
										.getSeconds()
										.toLocaleString("en-US", {
											minimumIntegerDigits: 2,
											useGrouping: false,
									...
									dicomDictArray[i].dict[dicomTag].Value[0] =
										`${hoursWithOffset}${minutesWithOffset}${secondsWithOffset}`;
									dicomTagValuesReplacedNum++;
								}
							}
						}
					} else if (
						dicomTagToNemaActionObject[dicomTag][6] === "K" &&
						retainDescriptorsInputCheckbox.checked
					) {
					} else {
						if (dicomTag in dicomDictArray[i].dict) {
				...
				hashCode(JSON.stringify(dicomDictArray[i])).then((hex) => {
					...
					const dicomTagSavePathArray = dicomTagSavePathInputText.value
						.slice(0, -1)
						.split("/");
					...
					for (let j = 1; j < dicomTagValueSavePathArray.length; j++) {
						zip.file(
							`${dicomTagValueSavePathArray.slice(0, j).join("/")}/`,
							"",
							{
								date: date,
							},
					...
					if (i === filesNum - 1) {
						...
						dicomTagValuesReplacedNumSpan.textContent =
							dicomTagValuesReplacedNum;
