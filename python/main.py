...
class TestWebApplication(unittest.TestCase):
    def setUp(self: "TestWebApplication") -> None:
        ...
        self.generated_data_file_path = generated_data_file_path
    ...
    def test_web_application(self: "TestWebApplication") -> None:
        with sync_playwright() as playwright:
            ...
            only_files_generated_data_file_path = [
                self.generated_data_file_path / file
                for file in os.listdir(self.generated_data_file_path)
                if (self.generated_data_file_path / file).is_file()
            ]
            page.set_input_files(
                "#load-directory-input-file",
                sorted(only_files_generated_data_file_path),
            )
            ...
            with Path("bin/screenshot.png").open("rb") as file:
                assert (
                    hashlib.sha256(file.read()).hexdigest()
                    == "6a2288402832ba59d081e824ce476b018f8782f89612d0797e1d425011514be7"  # noqa: E501
                )
            context.close()
            browser.close()
